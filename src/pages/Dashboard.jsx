  ...
  for(let i=0; i<10; i++){
    ...
    if(is5){
      ...
      if(stamped){
        ...
        ctx.fillText('tocá para',cx,cy+2)
        ctx.fillText('editar',cx,cy+14)
      }
    }
  }
...
export default function Dashboard() {
  ...
  function handleCanvasClick(e) {
    ...
    const currentDate = sellos[idx] || todayISO
    setSelloEdit({ index: idx, date: currentDate })
  }

  async function handleSelloEditSave() {
    if(!selloEdit || !cardClient) return
    const sellos = cardClient.sellos?.length ? [...cardClient.sellos].sort() : [cardClient.fechaCorte]
    const newSellos = [...sellos]
    newSellos[selloEdit.index] = selloEdit.date
    const sorted = [...newSellos].sort()
    const newFecha = sorted[sorted.length-1]
    try {
      await updateDoc(doc(db,'clientes',cardClient.id),{ sellos:sorted, fechaCorte:newFecha })
      setSelloEdit(null)
    } catch(err){ alert('Error: '+err.message) }
  }
  ...
  async function handleSelloEditDelete() {
    ...
    const newFecha = newSellos[newSellos.length-1]
    try {
      await updateDoc(doc(db,'clientes',cardClient.id),{ sellos:newSellos, fechaCorte:newFecha })
      setSelloEdit(null)
    } catch(err){ alert('Error: '+err.message) }
  }
